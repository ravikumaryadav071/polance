...
if($user->isLoggedIn()){
	...
	if(isset($_POST) and !empty($_POST)){
		...
		if($add_tags != ""){
			...
			if(isset($_FILES) && !empty($_FILES)){
				...
				if(isset($_FILES['file']['name']) && (count($_FILES['file']['name'])>0) ){
					...
					$db_up->query("SELECT id FROM {$user_data->username}_posts ORDER BY id DESC, LIMIT 0,1", array(), 'SELECT *');
					$id = $db_up->count();
					...
					for($i=0; $i<count($_FILES['file']['name']); $i++){
						...
						if($_FILES['file']['name'] != ''){
							...
							if($file_name && ((in_array($file_extention, $valid_ext) && in_array($type, $valid_formats)))){
								...
								$target = "posts/".strtolower(substr($user_data->username, 0, 1))."/$user_data->username/";
					...
					$db_up->insert($user_data->username.'_posts', array('text_content'=>$post_text, 'file_type'=>$add_files_formats, 'extention'=>$add_files_etx, 'files'=>$add_files_paths, 'contributors'=>$add_contris, 'refs'=>$add_refs, 'interest_tags'=>$add_tag_id, 'privacy'=>strtoupper($privacy)));
					...
					$db_up->insert($user_data->username.'_posts', array('text_content'=>$post_text, 'contributors'=>$add_contris, 'refs'=>$add_refs, 'interest_tags'=>$add_tag_id, 'privacy'=>strtoupper($privacy)));
			...
			if(isset($_POST['catagory_id']) && isset($_POST['interest_id'])){
				$catagory_id = $_POST['catagory_id'];
				$interest_id = $_POST['interest_id'];
				$tot_int = count($catagory_id);
				$tot_cat = count($catagory_id);
		...
		if(isset($last_res) && !empty($last_res)){
			...
			if(isset($contris) && !empty($contris)){
				...
				for($i=0; $i<$tot_contris; $i++){
					...
					if($contri!="" && !empty($contri)){
						...
						$db_un->insert($c_user_data->username.'_notifications', array('userid'=>$user_data->id, 'post_id'=>$last_res->id, 'ntf_type'=>'USER', 'sec_userid'=>$user_data->id, 'action'=>'CONTRIBUTED'));
			...
			$db_upr->query("CREATE TABLE {$user_data->username}_{$last_res->id}_upvotes (userid int not null primary key)", array(), 'CREATE');
			$db_upr->query("CREATE TABLE {$user_data->username}_{$last_res->id}_downvotes (userid int not null primary key)", array(), 'CREATE');
			$db_upr->query("CREATE TABLE {$user_data->username}_{$last_res->id}_shares (userid int not null primary key)", array(), 'CREATE');
			$db_upr->query("CREATE TABLE {$user_data->username}_{$last_res->id}_reports (userid int not null primary key)", array(), 'CREATE');
			$db_upr->query("CREATE TABLE {$user_data->username}_{$last_res->id}_varify (userid int not null primary key)", array(), 'CREATE');
			$db_upr->query("CREATE TABLE {$user_data->username}_{$last_res->id}_collects (userid int not null primary key)", array(), 'CREATE');
			$db_upc->query("CREATE TABLE {$user_data->username}_{$last_res->id}_comments (id int auto_increment not null primary key, userid int not null, comment text not null, deleted varchar(3) not null default 'NO', tot_reports int not null, date timestamp not null default CURRENT_TIMESTAMP)", array(), 'CREATE');
		}
