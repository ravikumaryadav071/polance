...
class DBusers_interests{
	...
	private function __construct(){
		...
		try{
			$this->_pdo = new PDO('mysql:host=' . config::get('mysql_for_users_interests/host') .';dbname=' . config::get('mysql_for_users_interests/db'), config::get('mysql_for_users_interests/username'), config::get('mysql_for_users_interests/password'));
	...
	public static function getInstance(){
		...
		if(!isset(self::$_instance)){
			self::$_instance = new DBusers_interests();
		}
	...
	public function query($sql, $params = array(), $operation){
		...
		if($this->_query = $this->_pdo->prepare($sql)){
			...
			if($this->_query->execute()){
				...
				$this->_error = true;
				$this->_ei = $this->_pdo->errorInfo();
				$this->_ec = $this->_pdo->errorCode();
			}
		}
