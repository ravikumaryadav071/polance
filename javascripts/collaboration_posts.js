$(document).ready(function(){
	...
	$('body').on('keyup', '#sugst_contri_user', function(){
		...
		if(sugst_user != ''){
			...
			$.ajax({
				...
				url: 'php_response_to_ajax/col_contributors_suggestions.php?search_user='+sugst_user+'&col='+col_name,
				type: 'POST',
				data: false,
				cache: false,
				contentType: false,
	...
	$('body').on('click', '#add_tag', function(){
		...
		$(this).hide();
		$(this).parent().slideUp(500);
		$(this).parent().find('#cat_id').attr('name', 'catagory_id[]');
		$(this).parent().find('#int_id').attr('name', 'interest_id[]');
		$(this).parent().find('#interest_name').attr('name', 'interest_name[]');
		$(this).parent().append('<input type="button" id="remove_tag" value="REMOVE">').show();
		var add_int =  $(this).parent().html();
		var passed = true;
		$('body #added_tags div').each(function(){
			if($(this).html() == add_int){
				passed = false;
			}
		});

		if(passed){
			add_tag_area = $('body #added_tags div:first');
			if( add_tag_area.html() != undefined){
				$(add_tag_area).before('<div>'+add_int+'</div>').slideDown(500);
			}else{
				$('body #added_tags').append('<div>'+add_int+'</div>').slideDown(500);
			}
		}
	...
	$('body').on('click', '#remove_tag', function(){
		...
		$(this).hide();
		$(this).parent().slideUp(500);
		$(this).parent().find('#cat_id').removeAttr('name');
		$(this).parent().find('#int_id').removeAttr('name');
		$(this).parent().find('#interest_name').removeAttr('name');
		$(this).parent().append('<input type="button" id="add_tag" value="ADD">').show();
		var remove_int =  $(this).parent().html();
		var passed = true;

		$('body #all_tags div').each(function(){
			if($(this).html() == remove_int){
				passed = false;
			}
		});
		
		if(passed){
			$('body #all_tags').append('<div>'+remove_int+'</div>').slideDown(500);
		}
		myParent = $(this).parent();
		$(this).remove();
		setTimeout(function(){
