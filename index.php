...
<html>
	...
	<body>
		...
		if ($user->isLoggedIn()) {
			...
			<div id="search_form">
				<input type="button" id="popup_search" value="Search">
			...
			$_SESSION['updated_feeds'] = array();
			$_SESSION['updated_feeds']['users'] = array();
			$_SESSION['updated_feeds']['collaboration'] = array();
			
			//$_SESSION['updated_feeds']['users'] = $_SESSION['updated_feeds']['users'];
			//$_SESSION['updated_feeds']['collaboration'] = $_SESSION['updated_feeds']['collaboration'];
			...
			do{
				...
				if($tot_users>0){
					...
					for($i=0; $i<$tot_users; $i++){
						...
						if($db_uf->count()==0){
							if(!in_array($f_user->userid, $_SESSION['updated_feeds']['users'])){
								$_SESSION['updated_feeds']['users'][] = $f_user->userid;
			...
			do{
				...
				if($tot_cols>0){
					...
					for($i=0; $i<$tot_cols; $i++){
						$col = $cols[$i];
						if(!in_array($col->col_id, $_SESSION['updated_feeds']['collaboration'])){
							$_SESSION['updated_feeds']['collaboration'][] = $col->col_id;
		...
		<script type="text/javascript" src="javascripts/popup_search.js"></script>
