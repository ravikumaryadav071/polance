...
// 	$db_cp->get($unique_name.'_edited_posts', array('id', '=', $post_info->edit_id));
...
if(isset($post_info)){
	...
	$db_cpr->get("{$unique_name}_{$post_info->id}_upvotes", array('userid', '=', $user_data->id));
	$upvoted = $db_cpr->count();

	$db_cpr->get("{$unique_name}_{$post_info->id}_downvotes", array('userid', '=', $user_data->id));
	$downvoted = $db_cpr->count();

	$db_cpr->get("{$unique_name}_{$post_info->id}_shares", array('userid', '=', $user_data->id));
	$shared = $db_cpr->count();

	$db_cpr->get("{$unique_name}_{$post_info->id}_reports", array('userid', '=', $user_data->id));
	$reported = $db_cpr->count();

	$db_cpr->get("{$unique_name}_{$post_info->id}_varify", array('userid', '=', $user_data->id));
	$varified = $db_cpr->count();
	...
	?>
	<div class="checkborder" <?php if(isset($stv)){ ?>id="<?php echo $stv; ?>"<?php } ?>>
		<?php
		...
		<div id="tags">
			...
			for($j=0; $j<count($tag_links); $j++){
				...
				if(isset($tag_links[$j])){
					...
					if($j==1){
						?>
						<a href="javascript: void(0)" id="show_tags">Show More</a>
						<div id="show_more_tags" hidden="hidden">
						<?php	
					}
					if($j>0){
						?>
			...
			<div id="contris">
				...
				for($j=0; $j<count($show_contris); $j++){
					if($show_contris[$j] != ""){
						...
						if($j==1){
							?>
							<a href="javascript: void(0)" id="show_contris">Show More</a>
							<div id="show_more_contris" hidden="hidden">
							<?php
						}
						if($j>0){
							?>
		...
		if(count($show_refs)){
			...
			<div id="references">
				...
				for($j=0; $j<count($show_refs); $j++){
					...
					if($j==1){
						?>
						<a href="javascript: void(0)" id="show_refs">Show More</a>
						<div id="show_more_refs" hidden="hidden">
						<?php
					}
					if($j>0){
						?>
		...
		?>
		<div id="counts">Upvotes: <a href="javascript: void(0)"><span id="up_count"><?php echo $up_count; ?></span></a> | Downvotes: <a href="javascript: void(0)"><span id="down_count"><?php echo $down_count; ?></span></a> | Shares: <a href="javascript: void(0)"><span id="share_count"><?php echo $share_count; ?></span></a> | Varifications: <a href="javascript: void(0)"><span id="varify_count"><?php echo $varify_count; ?></span></a> | Collect: <a href="javascript: void(0)"><span id="collect_count"><?php echo $collect_count; ?></span></a> | Comments: <a href="javascript: void(0)"><span id="comment_count"><?php echo $comment_count; ?></span></a></div>
		<div id="user_response">
			<?php
			if($upvoted>0){
				?>
				<input type="button" id="col_upvote" value="Upvote" hidden="hidden" <?php if($downvoted>0){ ?>disabled="disabled"<?php } ?>>
				<input type="button" id="col_upvoted" value="Upvoted" <?php if($downvoted>0){ ?>disabled="disabled"<?php } ?>>
				<?php
			}else{
				?>
				<input type="button" id="col_upvote" value="Upvote" <?php if($downvoted>0){ ?>disabled="disabled"<?php } ?>>
				<input type="button" id="col_upvoted" value="Upvoted" hidden="hidden" <?php if($downvoted>0){ ?>disabled="disabled"<?php } ?>>
				<?php
			}
			if($downvoted>0){
				?>
				<input type="button" id="col_downvote" value="Downvote" hidden="hidden" <?php if($upvoted>0){ ?>disabled="disabled"<?php } ?>>
				<input type="button" id="col_downvoted" value="Downvoted" <?php if($upvoted>0){ ?>disabled="disabled"<?php } ?>>
				<?php
			}else{
				?>
				<input type="button" id="col_downvote" value="Downvote" <?php if($upvoted>0){ ?>disabled="disabled"<?php } ?>>
				<input type="button" id="col_downvoted" value="Downvoted" hidden="hidden" <?php if($upvoted>0){ ?>disabled="disabled"<?php } ?>>
				<?php
			}
			if($shared>0){
				?>
				<input type="button" id="col_share" value="share" hidden="hidden">
				<input type="button" id="col_shared" value="shared">
				<?php
			}else{
				?>
				<input type="button" id="col_share" value="share">
				<input type="button" id="col_shared" value="shared" hidden="hidden">
				<?php
			}
			if($varified>0){
				?>
				<input type="button" id="col_varify" value="Verify" hidden="hidden">
				<input type="button" id="col_varified" value="Verifed">
				<?php
			}else{
				?>
				<input type="button" id="col_varify" value="Verify">
				<input type="button" id="col_varified" value="Verifed" hidden="hidden">
				<?php
			}
			if($reported>0){
				?>
				<input type="button" id="col_report" value="Report" hidden="hidden">
				<input type="button" id="col_reported" value="Reported">
				<?php
			}else{
				?>
				<input type="button" id="col_report" value="Report">
				<input type="button" id="col_reported" value="Reported" hidden="hidden">
				<?php
			}
			?>
			<input type="button" id="collect" value="Collect">
			<input type="checkbox" id="collect_cb" hidden="hidden">
			<input type="button" id="col_comment" value="Comment">
			<input type="hidden" id="col_post_id" value="<?php echo $post_info->id; ?>">
			<input type="hidden" id="col_name" value="<?php echo $unique_name; ?>">
			<div id="comment_here" hidden="hidden">
				<div id="show_comment_area">
					<a href="javascript: void(0)" id="show_col_comments">Show Comments</a>
				</div>
				<form id="col_comment_form" method="post" action="">
					<textarea name="comment_text" id="comment_text" value=""></textarea>
					<input type="hidden" name="post_id" id="c_col_post_id" value="<?php echo $post_info->id; ?>">
					<input type="hidden" name="col_name" id="c_col_name" value="<?php echo $unique_name; ?>">
